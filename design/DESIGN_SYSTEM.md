...
> ⚠️ **`design/workbench.html` 现在有两套令牌层，别按本节去改它。**
> 2026-09-20 起，那个原型的材质与颜色改由 **`src/styles/prism.css`** 提供
> （《Prism 设计令牌规范》`design/DESIGN_TOKENS.md` 的实现）：
> 材质是 `.g` + `.g1~.g4` 而不是 `glass-*`，九个菜单域各有自己的一组色。
> 它由 `design/build-css.mjs` 内联成两个 style 块：
> `style#lucky-y-tw`（本文件描述的布局工具类）+ `style#lucky-y-prism`（Prism 令牌）。
> **本文仍适用于 `src/**` 里的组件** —— 两者尚未合并，
> 合并顺序与差异清单见 `DESIGN_TOKENS.md` §8。
...
**反向也成立（实底按钮）：彩色实底上放白字时，底色同样只能取 `-strong` / `-500` 档。**

| 实底 | 白字 | 实测 |
| --- | --- | --- |
| `--color-life` #0f9d76 | ❌ | **3.49:1** |
| `--color-life-strong` #0a6b52 | ✅ | **6.48:1** |
| `--color-work-strong` #1d4fb8 | ✅ | 7.34:1 |
| `--color-learn-strong` #5b21b6 | ✅ | 8.98:1 |
| `--color-brand-500` #4f46e5 | ✅ | 6.29:1 |

判据一句话：**只要是"用来承载文字的色"，无论它当背景还是当前景，都往深的那一档取。**
`--accent`（= `brand-400` / 各域主色阶）是给**装饰**用的（光晕 / 描边 / 色条 / 圆点），
`--accent-deep`（= `brand-500` / 各域 `-strong`）才是给**实底**用的 ——
工作台那枚「进入当前模块主页面」的 CTA 就是按这条取的 `--accent-deep`。

两条从这条派生出来的补充判据（都是算出来的，不是试出来的）：

- **渐变实底：起点必须是深档。** 试过让 CTA 用 `--accent → --accent-deep` 的竖渐变（想亮一档、更贴环境）
  → **life 档在文字顶端只剩 4.18:1**（文字在垂直中段，那里浅档还没衰减完）。
- **上沿来光必须停在文字上沿之前。** CTA 顶上那层白 16% 的来光收在 **42%** 处，
  于是白字整段落在纯底色上（实测文字行盒顶端 α=0.071，四套仍有 5.47 / 5.56 / 6.25 / 7.71）。
  光带往下多延伸一点就压到字上了，**而肉眼完全看不出区别** —— 这条只能用百分比钉住。
- **底衬会变时，前景色不要跟它同源。** CTA 的 self 态是域色薄染的玻璃片，它背后是随模块换色的
  Hero 底色带 → 文字用中性 `ink-700`（≈7.2:1），**不用域色 `-strong`**（在同色薄染上只有约 4.3:1）。
  同源就等于把对比度押在"底衬恰好够亮"上。
...
| **深色实底按钮的禁用态用 `opacity`** | 按钮化成一块浅色块，白字跟着一起变淡 → 实测只剩约 **2.0:1**，读起来像渲染坏了，而不是"禁用" | 透明度会把**底衬和文字同时**推向背景色，两者之间的对比度一起塌掉。**浅底深字**才适合压透明度（`LoginPage` 的 `disabled:opacity-60` 属于这种）；**深底白字要"换填充"** —— 但**别换成中性灰**（那是"坏掉的按钮"的观感），换成**域色薄染的玻璃片**：`color-mix(in srgb, var(--accent) 16%, transparent)` + 域色发丝 + 中性 `ink-700` 文字（≈7.2:1），形状/尺寸/位置全部保留 |
| **实底按钮的上沿来光压到文字上** | 白字对比度悄悄掉档（肉眼看不出，只有算过才知道） | 白 16% 的来光必须**在文字上沿之前收到 0**（本例 42% 处）。同理，整块变亮 / 大面积扫光都会**压过白字** —— hover 的动效只放在不承载文字的地方（外圈光环 / 光晕 / 位移 / 箭头） |
| 禁用/当前态用 `display:none` 处理 | 切换模块时 Hero 高矮变化，下方内容整块跳动 | "只换内容、不换显隐"才是重置；**占位本身就是信息**（"这里本来有一枚按钮"） |
| **在浅色表面上用低 α 的白描边** | 白线糊进底衬，读成一团印子（放大 8× 才看得出来） | **装饰的可见性必须在它真正所处的底衬上验证**。同一条 `rgba(255,255,255,.42)`，压在深色实底上很清楚、压在 α.70 的浅色玻璃上等于没画 —— 换个位置就从"描边"退化成"污渍"，而截图对比时极容易被当成"这条规则没生效"（正解是：先把背景临时换成纯色验证几何，再回来调 α） |
...
**头像 / 圆标**（是一颗**球**，不是"圆角方块铺一条渐变"）

```css
border-radius: 999px;
background:
  radial-gradient(circle at 32% 26%, rgba(255, 255, 255, .34), rgba(255, 255, 255, 0) 60%),
  linear-gradient(135deg, hsl(265, 72%, 64%), hsl(235, 68%, 52%));  /* = avatarGradient(265) */
box-shadow:
  inset 0 0 0 1px rgba(255, 255, 255, .58),  /* 细白环，把外缘从浅色玻璃上切出来 */
  0 6px 16px -6px rgba(79, 70, 229, .6);     /* 域色辉光，让它"浮"起来 */
```

> 体积感来自**那层径向高光，不来自圆角** —— 去掉它，同一组渐变色立刻退回成"标签"。
> 色相继续取 `src/data/derive.ts` 的 `avatarGradient(hue)`（与顶栏用户头像**同一个生成器**）：换的是光，不是色。
> ⚠️ 白环的 α 别往下调，见 §3.6 最后一条。
>
> 同族的**圆形图标按钮**（`rounded-full`，白底 + 品牌紫图标 + 细白环 + 柔和阴影）是这套"球"语汇的配角，
> 与球体头像同处一行时读作一组；**输入栏两端各住一枚圆钮**时，只把实底给主行动那一端（另一端中性）。
